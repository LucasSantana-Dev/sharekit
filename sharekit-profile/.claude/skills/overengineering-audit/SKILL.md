...
Done when: scope confirmed in output (e.g., "Auditing: src/payments/ | --changed | --category abstraction | or "ask for scope").
...
| --- | --- |
| "We might need it later" | Speculative features cost now (indirection, maintenance, cognitive load) with unknown future value. Add when demand is real, not imagined. |
| "This abstraction makes it more flexible" | Single-implementation abstractions are not flexible — they hide complexity behind a seam nobody can swap. Flexibility is justified when a 2nd implementation exists or is on the roadmap. |
| "It's just one extra layer" | Layers accumulate. Each hop in the call stack adds cognitive overhead, debugging friction, and reasoning depth. Justify each hop by a real seam or ownership boundary. |
| "The config makes it reusable" | One config value is a constant, not reusability. Introduce config when multiple concrete values are in use or planned, not for hypothetical variability. |
...
Done when: findings ranked by severity, each with verified implementor/caller count, cost itemized (files, hops, cognitive load), and simpler alternative stated.
