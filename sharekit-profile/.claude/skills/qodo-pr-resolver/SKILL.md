...
1. **PR Compliance Guide**  - Security/ticket/custom compliance with [green]/[yellow]/[red]/[white] indicators
...
- Inform: "[WARN] You have uncommitted changes. These won't be included in the Qodo review."
...
- Inform: "[WARN] You have N unpushed commits. Qodo hasn't reviewed them yet."
...
1. **Action level determines severity range:**
   - **"Action required"** issues → Can only be [red] CRITICAL or [orange] HIGH
   - **"Review recommended"** / **"Remediation recommended"** issues → Can only be [yellow] MEDIUM or [white] LOW
   - **"Other"** / **"Advisory comments"** issues → Always [white] LOW (lowest priority)
...
2. **Qodo's position within each action level determines the specific severity:**
   ...
   - All "Other" issues are treated as [white] LOW regardless of position

**Example:** 7 "Action required" issues would be split as:
- Issues 1-3: [red] CRITICAL
- Issues 4-7: [orange] HIGH
...
- Issues 1-2 or 1-3: [red] CRITICAL (first ~half of "Action required")
- Issues 3-5 or 4-5: [orange] HIGH (second ~half of "Action required")
- Issues 6-7: [yellow] MEDIUM (first ~half of "Review recommended")
- Issue 8: [white] LOW (second ~half of "Review recommended")
- Issues 9-10: [white] LOW (all "Other" issues)

**Action guidelines:**
- [red] CRITICAL / [orange] HIGH ("Action required"): Always "Fix"
- [yellow] MEDIUM ("Review recommended"): Usually "Fix", can "Defer" if low impact
- [white] LOW ("Review recommended" or "Other"): Can be "Defer" unless quick to fix; "Other" issues are lowest priority

#### Output format

**IMPORTANT: Use actual Unicode emoji characters** (e.g. `[red]`, `[orange]`, ``, ``, ``), NOT GitHub-style shortcodes (`:red_circle:`, `:books:`, `:shield:`). Shortcodes do not render in terminal environments.
...
|---|----------|-------------|---------------|------|--------|
| 1 | [red] CRITICAL | Insecure authentication check | • **Location:** src/auth/service.py:42<br><br>• **Issue:** Authorization logic is inverted |  Bug  Security | Fix |
| 2 | [red] CRITICAL | Missing input validation | • **Location:** src/api/handlers.py:156<br><br>• **Issue:** User input not sanitized before database query |  Rule violation  Reliability | Fix |
| 3 | [orange] HIGH | Database query not awaited | • **Location:** src/db/repository.py:89<br><br>• **Issue:** Async call missing await keyword |  Bug ✓ Correctness | Fix |
```
...
- [FAIL] "Cancel" - Exit without making changes
  ...
  - **Present the fix and ask for approval in a SINGLE step:**
    ...
    5. Immediately use AskUserQuestion with these options:
       - If the issue's Action is **"Fix"** (default for CRITICAL/HIGH, and most MEDIUM):
         - [OK] "Apply fix" - Apply the proposed change
       ...
       - If the issue's Action is **"Defer"** (only reachable in single-finding mode):
         - ⏭ "Confirm defer" - Keep the deferral (will prompt for reason)
         - [OK] "Apply fix anyway" - Apply the proposed change despite the suggested deferral
         -  "Modify" - User wants to adjust the fix first
...
**Example:** Show location, Qodo's guidance, current code, proposed diff, then AskUserQuestion with options ([OK] Apply fix / ⏭ Defer /  Modify). Wait for user choice, apply via Edit tool if approved.
...
- For each issue marked as "Fix" (starting with CRITICAL):
  ...
  - Report each fix with the agent prompt that was followed:
    > [OK] **Fixed: [Issue Title]** at `[Location]`
