...
if [[ -n $MUSIC ]]; then
  ...
  "$FFMPEG" -y -i "$IN" -stream_loop -1 -i "$MUSIC" \
    -filter_complex "[0:v]$VFADE[v];[0:a]asplit=2[voice][sc];[1:a]volume=$ML[m];[m][sc]sidechaincompress=threshold=$DT:ratio=$DR:attack=50:release=500[duck];[voice][duck]amix=inputs=2:duration=first:dropout_transition=2,loudnorm=I=-14:TP=-1.5:LRA=11,$AFADE[a]" \
