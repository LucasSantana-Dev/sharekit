...
Query prior error-handling patterns in this codebase:
- **Check codebase-memory** via `codebase-memory-mcp` for framework/library patterns (pooled resources, auto-cleanup, supervisor config).
- **Check graphify** if a knowledge graph exists: `graphify query "error handling patterns, resource cleanup, framework auto-cleanup" --budget 300`.

**Done when:** cached framework patterns surfaced (e.g. "Prisma pooling, discord.js auto-cleanup") OR no match found (proceed with default don't-flag list).

Skip if: audit is scoped to `--changed` on a small diff (framework patterns unlikely to shift) or on a previously-audited codebase within 24h.
...
**Done when:** scope stated in one sentence; scope narrower than the entire repo; user intent and scope match.
...
| --- | --- |
| "This catch is fine — it's just logging, and the app continues normally." | Execution continues in an *invalid* state if that catch doesn't abort or return. Verify no code below the catch uses the failed value; if it does, that's a Tier 2 finding even if it "seems to work." |
| "The pool handles cleanup, so no resource leak here." | Correct *if* you used a pooled client (Prisma, ioredis, pg.Pool). But if this file's init manually opens a connection/socket/file without a pool, verify the cleanup is actually on the error path, not just the happy path. |
| "This is in a supervisor (PM2/Docker/k8s), so uncaught exceptions are fine." | Process restart ≠ graceful resource release. Timers still pending on the process when it's killed, listeners still open, pending writes may not flush. Not a leak finding, but note it for operational completeness if the codebase claims to clean up. |
| "Grepping for 'close' / 'cleanup' shows it happens somewhere in the module." | Grep confirms *existence*, not *reachability on the error path*. Verify the cleanup is in a `finally` or error handler, not just a happy-path return. "Somewhere" is not the same as "on all paths." |

## Done-When Checklist

- **Scope declared:** one-line statement of the chosen scope (`file`, `--changed`, `--category`, `--severity`, `--budget`).
- **Framework patterns sourced:** RAG pre-flight executed (or skipped with justification); framework auto-cleanup list confirmed.
- **Detection catalog applied:** all scoped code checked against Tier 1 (high-signal) and Tier 2 (corroborating-context) patterns.
- **Cleanup paths verified:** for each suspected leak, grep/code-read confirms cleanup is NOT reachable on the error path (not just present in the file).
- **False positives filtered:** don't-flag list applied (pooled resources, auto-cleaned frameworks, supervisor-managed processes, intentional fire-and-forget, defensive logging, app-lifetime timers).
- **Verdict issued:** counts and severity summary (e.g., "1 HIGH, 2 MED") or "Clean" if no findings.
- **Evidence attached per finding:** file:line, the pattern name, the fix, and the check run to verify (not speculated).
...
**Done when:** verdict issued with severity counts and scope; all findings ≥MEDIUM have file:line + pattern name + fix + evidence (not speculation); low-confidence or false-positive findings filtered; next action surfaced (e.g., "route to `/refactor` for the HIGH finding" or "clean — no action needed").
