...
test("init scaffolds a publishable profile from source root", () => {
  ...
  fs.writeFileSync(path.join(sourceRoot, ".claude", "skills", "foo", "SKILL.md"), "foo skill content");
  fs.mkdirSync(path.join(sourceRoot, ".claude", "skills", "foo", "subdir"), { recursive: true });
  fs.writeFileSync(path.join(sourceRoot, ".claude", "skills", "foo", "subdir", "nested.txt"), "nested");
  ...
  assert.equal(fs.readFileSync(path.join(profileDir, "claude", "skills", "foo", "SKILL.md"), "utf8"), "foo skill content");
  assert.equal(
    fs.readFileSync(path.join(profileDir, "claude", "skills", "foo", "subdir", "nested.txt"), "utf8"),
    "nested",
  );
